...
3. If your machine doesn't have `git` and `vim`, type `./0gitvim`. Most Ubuntu systems I've looked at have them both, but my Linux Mint 18 system did not.
3. If you haven't done a software upgrade in a while, do it now with `./0update-upgrade`. You will need to reboot after it completes.
3. Type `./1core`. This will install a few core utilities.
5. The scripts are modular - you only need to install what you're going to use.
