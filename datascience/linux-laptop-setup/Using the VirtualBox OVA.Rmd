---
title: "Using the Hack Oregon Base v2 VirtualBox OVA"
...
7. The box ships with a publicly-known password, `ORturkeyeggs`, for the `vagrant` account. You will need to change it! Enter `passwd`.
    ```
    $ passwd
...
2. Enter `ssh vagrant@localhost -p 2222`. You will be asked for the new password you defined above.
...
3. Enter `sudo shutdown -h now` to shut down the virtual machine. You can close the host terminal.
