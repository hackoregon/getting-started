...
6. Enter `vagrant up --provision --provider virtualbox`. You'll see
    ...
    ```
7. Enter `vagrant ssh`. You'll be inside the box. Enter 
    ```
    ...
    This updates the `getting-started` repository to the latest release and confgures the PostgreSQL database. You will be asked to set a PostgreSQL password for the `vagrant` user and to enter that password in a file, so you can access the database as `vagrant` without exposing your password!

7. Enter `activate-data-science` and then `jupyternb`. You'll see
    ```
